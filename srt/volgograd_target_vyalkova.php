<?php
require 'config.php';
// Волгоград.Таргет.Вялкова — Senler — 26.02.2026 18:52
...
if (!defined('BITRIX_WEBHOOK_1')) die('Ошибка: вебхук не настроен');
// Отправка на несколько хуков
$webhooks = array_filter([defined('BITRIX_WEBHOOK_1') ? BITRIX_WEBHOOK_1 : '', defined('BITRIX_WEBHOOK_2') ? BITRIX_WEBHOOK_2 : '']);
$hookName = 'volgograd_target_vyalkova';
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: Волгоград.Таргет.Вялкова\n";
    $securityLog .= "Hook: volgograd_target_vyalkova\n";
...
// ========================================
$vkData = json_decode($rawInput, true);
if (isset($vkData['type']) && isset($vkData['group_id'])) {
    // Подтверждение сервера
    if ($vkData['type'] === 'confirmation') {
        exit('7cf29047');
    }
    // Проверка секретного ключа
    if (($vkData['secret'] ?? '') !== 'yRpjsWniLxAgmtzhI') {
        http_response_code(403);
        exit('Invalid secret key');
    }
    // Обработка новых лидов
    if ($vkData['type'] === 'lead_forms_new') {
        $lead = $vkData['object'];
        $formId = $lead['form_id'];
        // Фильтрация по ID формы
        $allowedForms = ['5'];
        if (!in_array($formId, $allowedForms)) {
            exit('ok'); // Игнорируем чужие формы
        }
        // Парсинг ответов
        $answers = [];
        $vkQuiz = []; // Массив для вопросов и ответов (для комментариев)
        if (isset($lead['answers']) && is_array($lead['answers'])) {
            foreach ($lead['answers'] as $ans) {
                $key = $ans['key'];
                $question = $ans['question'];
                $answer = $ans['answer'];
                $answers[$key] = $answer;
                
                // Добавляем в массив для комментариев
                $vkQuiz[] = ['question' => $question, 'answer' => $answer];
                
                // Сопоставляем стандартные поля
                if ($key === 'first_name') {
                    $_POST['first_name'] = $answer;
                    $_POST['name'] = $answer;
                } elseif ($key === 'phone_number') {
                    $_POST['phone_number'] = $answer;
                    $_POST['phone'] = $answer;
                } elseif ($key === 'email') {
                    $_POST['email'] = $answer;
                } else {
                    // Остальные поля добавляем в POST для стандартной обработки
                    $_POST[$key] = $answer;
                }
            }
        }
        $_POST['form_name'] = $lead['form_name'] ?? '';
        $_POST['form_id'] = $formId;
        $_POST['group_id'] = $lead['group_id'] ?? '';
        $_POST['ad_id'] = $lead['ad_id'] ?? '';
        $_POST['answers'] = json_encode($answers, JSON_UNESCAPED_UNICODE);
        $_POST['vk_quiz'] = $vkQuiz;
        // Отвечаем ВК, что приняли
        echo 'ok';
        // Закрываем соединение для ВК, продолжаем работу скрипта
        if (function_exists('fastcgi_finish_request')) {
            fastcgi_finish_request();
        } else {
            // Fallback для Apache/Windows
            if (ob_get_level() > 0) {
                 ob_end_flush();
            }
            flush();
...
// SENLER HANDLER
// ========================================
$senlerData = json_decode($rawInput, true);
    // Идемпотентность (защита от дублей)
    $uniqBits = [
        (string)($senlerData['object']['vk_user_id'] ?? ''),
        (string)($senlerData['object']['unixtime'] ?? ''),
        (string)($senlerData['type'] ?? ''),
        (string)($senlerData['object']['date'] ?? ''),
    ];
    $idemKey = hash('sha256', implode('|', $uniqBits) . '|' . $rawInput);
    $idemDir = __DIR__ . '/../logs/idempotency/' . date('Y-m');
    if (!is_dir($idemDir)) @mkdir($idemDir, 0775, true);
    $idemFile = $idemDir . '/' . $idemKey . '.lock';

    if (file_exists($idemFile)) {
        echo json_encode(['status' => 'ok', 'duplicate' => true]);
        exit;
    }
    @file_put_contents($idemFile, date('c'));

    if (isset($senlerData['object']) && is_array($senlerData['object'])) {
    $obj = $senlerData['object'];
    // Name
    $firstName = trim((string)($obj['first_name'] ?? ''));
    $lastName = trim((string)($obj['last_name'] ?? ''));
    $input['client']['name'] = trim($firstName . ' ' . $lastName);

    // Birthdate
    $bdate = trim((string)($obj['bdate'] ?? ''));
    if ($bdate !== '') {
        $bdateParts = explode('.', $bdate);
        if (count($bdateParts) === 3) {
            // Bitrix expects YYYY-MM-DD
            $_POST['bdate'] = $bdateParts[2] . '-' . $bdateParts[1] . '-' . $bdateParts[0];
        } else {
             $_POST['bdate'] = $bdate;
        }
    }

    // Phone search (scan variables)
    $variables = is_array($obj['variables'] ?? null) ? $obj['variables'] : [];
    $phoneKeys = ['nomer', 'number', 'phone', 'Phone', 'PHONE', 'telephone', 'tel', 'mobile', 'номер', 'телефон'];
    foreach ($phoneKeys as $pkey) {
        foreach ($variables as $varKey => $varValue) {
            if (strcasecmp((string)$varKey, $pkey) === 0 && is_scalar($varValue)) {
                $input['client']['phone'] = (string)$varValue;
                break 2;
            }
        }
    }

    // Map variables to form_data
    foreach ($variables as $k => $v) {
        if (is_scalar($v)) {
            $input['form_data'][] = ['name' => $k, 'value' => $v];
        }
    }

    // VK Link & User ID
    $vkUserId = $obj['vk_user_id'] ?? '';
    if ($vkUserId) {
        $input['form_data'][] = ['name' => 'VK User ID', 'value' => $vkUserId];
        $domain = $obj['domain'] ?? '';
        $vkLink = $domain ? 'https://vk.com/' . $domain : 'https://vk.com/id' . $vkUserId;
        $input['form_data'][] = ['name' => 'VK Link', 'value' => $vkLink];
    }

    // Message from subscriptions
    $subs = $obj['subscriptions'] ?? [];
    if (!empty($subs) && isset($subs[0]['message']['text'])) {
        $input['form_data'][] = ['name' => 'Сообщение', 'value' => $subs[0]['message']['text']];
    }
...
$input = [
    'client' => ['name' => '', 'phone' => '', 'email' => '', 'birthdate' => ''],
    'utm' => [],
    'form_data' => [],
    'page' => ['url' => '', 'name' => ''],
    'quiz' => [],
...
// Переносим вопросы из VK в input[quiz] для красивых комментариев
if (!empty($_POST['vk_quiz'])) {
    $input['quiz'] = $_POST['vk_quiz'];
}
...
// ========================================
// ОБРАБОТКА СТАНДАРТНОГО BITRIX WEBHOOK (fields[...])
// ========================================
if (isset($_POST['fields']) && is_array($_POST['fields'])) {
    $bxFields = $_POST['fields'];
    $input['raw_bitrix'] = $bxFields; // Сохраняем все поля для сквозной передачи

    // Name
    if (empty($input['client']['name']) && !empty($bxFields['NAME'])) {
         $input['client']['name'] = $bxFields['NAME'];
    }
    // Birthdate
    if (empty($input['client']['birthdate']) && !empty($bxFields['BIRTHDATE'])) {
         $input['client']['birthdate'] = $bxFields['BIRTHDATE'];
    }
    // Phone (Bitrix sends complex array, or string)
    if (empty($input['client']['phone']) && !empty($bxFields['PHONE'])) {
         $ph = $bxFields['PHONE'];
         if (is_array($ph)) {
             // Extract first value
             $first = reset($ph);
             $input['client']['phone'] = is_array($first) ? ($first['VALUE'] ?? '') : $first;
         } else {
             $input['client']['phone'] = $ph;
         }
    }
    // Email
    if (empty($input['client']['email']) && !empty($bxFields['EMAIL'])) {
         $em = $bxFields['EMAIL'];
         if (is_array($em)) {
             $first = reset($em);
             $input['client']['email'] = is_array($first) ? ($first['VALUE'] ?? '') : $first;
         } else {
             $input['client']['email'] = $em;
         }
    }
    // Comments
    if (!empty($bxFields['COMMENTS'])) {
         $input['form_data'][] = ['name' => 'Комментарий (Bitrix)', 'value' => $bxFields['COMMENTS']];
    }
    // UTM
    foreach (['UTM_SOURCE', 'UTM_MEDIUM', 'UTM_CAMPAIGN', 'UTM_CONTENT', 'UTM_TERM'] as $utm) {
        if (!empty($bxFields[$utm])) {
            $key = strtolower(substr($utm, 4));
            if (empty($input['utm'][$key])) {
                $input['utm'][$key] = $bxFields[$utm];
            }
        }
    }
}
...
// ========================================
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    // Проверка на специфичный тест Tilda (test=test, пустые поля)
    $isTildaTest = (isset($_REQUEST['test']) && $_REQUEST['test'] === 'test' && empty($input['client']['phone']));
    $logMsg = $isTildaTest ? "Тестовый запрос Tilda (OK, игнорируем ошибку валидации)\n" : "Тестовый запрос (OK)\n";
    file_put_contents(__DIR__ . '/../logs/volgograd_target_vyalkova.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
    if ($customResponseEnabled) {
        echo json_encode(['status' => 'ok'], JSON_UNESCAPED_UNICODE);
    } else {
        echo json_encode(['status' => 'ok', 'message' => 'Test successful'], JSON_UNESCAPED_UNICODE);
    }
    exit;
}
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/volgograd_target_vyalkova.log', $logEntry, FILE_APPEND);

    header('Content-Type: application/json; charset=utf-8');
    if ($customResponseEnabled) {
        http_response_code(400);
        echo json_encode(['error' => $phoneError], JSON_UNESCAPED_UNICODE);
    } else {
        echo json_encode([
            'status' => 'error',
            'lead_id' => 0,
...
$fields = [];
if (!empty($input['raw_bitrix'])) {
    $fields = $input['raw_bitrix'];
}
$comments = '';
...
$ch = curl_init();
// Отправка на каждый активный хук
$sendResults = [];
...
// Сохранение недоставленного лида
...
if ($deliveryFailed) {
    ...
    $failedFile = __DIR__ . '/../logs/volgograd_target_vyalkova_failed.json';
...
if (!isset($suppressResponse) || !$suppressResponse) {
    // Ответ сервису
    header('Content-Type: application/json; charset=utf-8');
    if ($customResponseEnabled) {
        if ($leadId) {
            echo json_encode(['id' => (string)$leadId], JSON_UNESCAPED_UNICODE);
        } else {
            $errCode = ($httpCode >= 400 && $httpCode < 600) ? $httpCode : 500;
            http_response_code($errCode);
            echo json_encode(['error' => $status], JSON_UNESCAPED_UNICODE);
        }
    } else {
        // Ответ: успех, если хотя бы один хук сработал
$anySuccess = array_filter($sendResults, fn($r) => $r['success']);
$leadId = $sendResults[0]['lead_id'] ?? null;

if (!empty($anySuccess) || empty($webhooks)) {
    // Ответ: успех, если хотя бы один хук сработал
$anySuccess = array_filter($sendResults, fn($r) => $r['success']);
$leadId = $sendResults[0]['lead_id'] ?? null;
...
if (!empty($anySuccess) || empty($webhooks)) {
    echo json_encode([
        'status' => $leadId ? 'ok' : 'warning',
        'lead_id' => $leadId,
        'message' => 'Отправлено в ' . count($webhooks) . ' вебхук(ов)',
        'hooks_sent' => count($webhooks)
    ], JSON_UNESCAPED_UNICODE);
} else {
    http_response_code(502);
    echo json_encode([
        'status' => 'error',
        'message' => 'Не удалось отправить ни в один вебхук'
    ], JSON_UNESCAPED_UNICODE);
}
} else {
    http_response_code(502);
