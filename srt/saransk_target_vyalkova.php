<?php
require 'config.php';
// Саранск. Таргет.Вялкова — Senler — 26.02.2026 18:44
...
$hookName = 'saransk_target_vyalkova';
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: Саранск. Таргет.Вялкова\n";
    $securityLog .= "Hook: saransk_target_vyalkova\n";
...
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    ...
    file_put_contents(__DIR__ . '/../logs/saransk_target_vyalkova.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/saransk_target_vyalkova.log', $logEntry, FILE_APPEND);
...
if ($deliveryFailed) {
    ...
    $failedFile = __DIR__ . '/../logs/saransk_target_vyalkova_failed.json';
