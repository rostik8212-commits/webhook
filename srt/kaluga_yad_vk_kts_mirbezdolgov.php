<?php
require 'config.php';
// Калуга. ЯД/ВК КЦ. МирБезДолгов — КЦ — 26.02.2026 16:08
...
$hookName = 'kaluga_yad_vk_kts_mirbezdolgov';
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: Калуга. ЯД/ВК КЦ. МирБезДолгов\n";
    $securityLog .= "Hook: kaluga_yad_vk_kts_mirbezdolgov\n";
...
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    ...
    file_put_contents(__DIR__ . '/../logs/kaluga_yad_vk_kts_mirbezdolgov.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/kaluga_yad_vk_kts_mirbezdolgov.log', $logEntry, FILE_APPEND);
...
if ($deliveryFailed) {
    ...
    $failedFile = __DIR__ . '/../logs/kaluga_yad_vk_kts_mirbezdolgov_failed.json';
