<?php
require 'config.php';
// Ульяновск.КЦ.Мир без долгов — КЦ — 26.02.2026 15:00
...
$hookName = 'ulyanovsk_kts_mir_bez_dolgov';
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: Ульяновск.КЦ.Мир без долгов\n";
    $securityLog .= "Hook: ulyanovsk_kts_mir_bez_dolgov\n";
...
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    ...
    file_put_contents(__DIR__ . '/../logs/ulyanovsk_kts_mir_bez_dolgov.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/ulyanovsk_kts_mir_bez_dolgov.log', $logEntry, FILE_APPEND);
...
if ($deliveryFailed) {
    ...
    $failedFile = __DIR__ . '/../logs/ulyanovsk_kts_mir_bez_dolgov_failed.json';
