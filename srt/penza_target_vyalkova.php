<?php
require 'config.php';
// Пенза. Таргет.Вялкова — Senler — 26.02.2026 17:45
...
$hookName = 'penza_target_vyalkova';
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: Пенза. Таргет.Вялкова\n";
    $securityLog .= "Hook: penza_target_vyalkova\n";
...
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    ...
    file_put_contents(__DIR__ . '/../logs/penza_target_vyalkova.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/penza_target_vyalkova.log', $logEntry, FILE_APPEND);
...
if ($deliveryFailed) {
    ...
    $failedFile = __DIR__ . '/../logs/penza_target_vyalkova_failed.json';
