<?php
// 1111 (копия) — Tilda — 27.02.2026 10:51
...
// Массив активных вебхуков
$webhooks = array_filter([
    defined('BITRIX_WEBHOOK_1') ? BITRIX_WEBHOOK_1 : '',
    defined('BITRIX_WEBHOOK_2') ? BITRIX_WEBHOOK_2 : ''
]);
$hookName = '1111_kopiya';
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: 1111 (копия)\n";
    $securityLog .= "Hook: 1111_kopiya\n";
...
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    ...
    file_put_contents(__DIR__ . '/../logs/1111_kopiya.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/1111_kopiya.log', $logEntry, FILE_APPEND);
...
$email = trim($input['client']['email'] ?? '');
if ($email !== '' && filter_var($email, FILTER_VALIDATE_EMAIL)) {
    $fields['EMAIL'] = [['VALUE' => $email, 'VALUE_TYPE' => 'WORK']];
}
// Поле utm_source -> UTM_SOURCE
$v = $_POST['utm_source'] ?? '';
if (is_array($v) && isset($v['value'])) $v = $v['value'];
if (is_array($v)) $v = reset($v) ?: '';
if (is_scalar($v) && trim((string)$v) !== '') $fields['UTM_SOURCE'] = trim((string)$v);
// Поле utm_medium -> UTM_MEDIUM
$v = $_POST['utm_medium'] ?? '';
if (is_array($v) && isset($v['value'])) $v = $v['value'];
if (is_array($v)) $v = reset($v) ?: '';
if (is_scalar($v) && trim((string)$v) !== '') $fields['UTM_MEDIUM'] = trim((string)$v);
// Поле utm_campaign -> UTM_CAMPAIGN
$v = $_POST['utm_campaign'] ?? '';
if (is_array($v) && isset($v['value'])) $v = $v['value'];
if (is_array($v)) $v = reset($v) ?: '';
if (is_scalar($v) && trim((string)$v) !== '') $fields['UTM_CAMPAIGN'] = trim((string)$v);
...
// ========================================
// ЛОГИРОВАНИЕ И ОТПРАВКА В BITRIX24 (НЕСКОЛЬКО ХУКОВ)
// ========================================
$logTime = date('d.m.Y H:i:s');
$baseLogEntry = "Время: {$logTime}\n";
$baseLogEntry .= "Сервис: Tilda\n";
$baseLogEntry .= "From: {$from}\n";
$baseLogEntry .= "Клиент: " . print_r($input['client'], true) . "\n";
$baseLogEntry .= "Город: " . ($input['city'] ?: 'не определён') . " (ID: " . ($cityId ?: 'не найден') . ")\n";
$baseLogEntry .= "RAW Input: " . substr($rawInput, 0, 1000) . "\n";
$baseLogEntry .= "POST: " . print_r($rawPost, true) . "\n";
$baseLogEntry .= "Quiz/Комментарии: " . print_r($input['quiz'], true) . "\n";
$baseLogEntry .= "Собранные комментарии (для лога): \n" . strip_tags(str_replace('<br>', "\n", $comments)) . "\n";
$baseLogEntry .= "Отправка комментариев в Bitrix: " . ($sendCommentsToBitrix ? 'ДА' : 'НЕТ') . "\n";
$baseLogEntry .= "Поля для Bitrix: " . print_r($fields, true) . "\n";
...
foreach ($sendResults as $res) {
    ...
    $logEntry .= "=== Хук #" . $res['hook_index'] . " ===\n";
    $logEntry .= "URL: " . $res['url'] . "\n";
    $logEntry .= "Статус: " . $res['error'] . "\n";
    $logEntry .= "Bitrix Response: " . substr($res['response'] ?? '', 0, 500) . "\n";
    $logEntry .= str_repeat('=', 50) . "\n\n";
    
    $logFile = __DIR__ . '/../logs/1111_kopiya_hook" . $res['hook_index'] . ".log';
    file_put_contents($logFile, $logEntry, FILE_APPEND);
}

// Сохраняем недоставленный лид, если ВСЕ хуки не сработали
$allFailed = array_filter($sendResults, fn($r) => $r['deliveryFailed']);
if (count($allFailed) === count($sendResults) && !empty($sendResults)) {
    $first = $sendResults[0];
    $failedLead = [
        'id' => uniqid('lead_'),
        'time' => $logTime,
    ...
    $failedFile = __DIR__ . '/../logs/1111_kopiya_failed.json';
...
// Ответ сервису (успех, если хотя бы один хук сработал)
if (!isset($suppressResponse) || !$suppressResponse) {
    ...
    if (!empty($anySuccess) || empty($webhooks)) {
        ...
        echo json_encode([
            ...
            'details' => array_map(fn($r) => ['hook' => $r['hook_index'], 'error' => $r['error']], $sendResults)
        ], JSON_UNESCAPED_UNICODE);
    }
}
