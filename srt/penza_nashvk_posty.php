<?php
require 'config.php';
// Пенза. НашВК. посты — ВКонтакте (Lead Forms) — 26.02.2026 16:32
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: Пенза. НашВК. посты\n";
    $securityLog .= "Hook: penza_nashvk_posty\n";
...
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    ...
    file_put_contents(__DIR__ . '/../logs/penza_nashvk_posty.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/penza_nashvk_posty.log', $logEntry, FILE_APPEND);
...
if ($deliveryFailed) {
    ...
    $failedFile = __DIR__ . '/../logs/penza_nashvk_posty_failed.json';
