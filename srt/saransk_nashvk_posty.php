<?php
require 'config.php';
// Саранск. НашВК.посты — ВКонтакте (Lead Forms) — 26.02.2026 16:35
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: Саранск. НашВК.посты\n";
    $securityLog .= "Hook: saransk_nashvk_posty\n";
...
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    ...
    file_put_contents(__DIR__ . '/../logs/saransk_nashvk_posty.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
...
if (!$phoneValid) {
    ...
    $logEntry .= "Сервис: ВКонтакте (Lead Forms)\n";
    ...
    file_put_contents(__DIR__ . '/../logs/saransk_nashvk_posty.log', $logEntry, FILE_APPEND);
...
$from = 'Заявка из ВКонтакте (Lead Forms)';
...
if ($deliveryFailed) {
    ...
    $failedFile = __DIR__ . '/../logs/saransk_nashvk_posty_failed.json';
