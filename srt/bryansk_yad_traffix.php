<?php
require 'config.php';
// Брянск. ЯД. TRAFFIX — Директ — 25.02.2026 17:01
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: Брянск. ЯД. TRAFFIX\n";
    $securityLog .= "Hook: bryansk_yad_traffix\n";
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/bryansk_yad_traffix.log', $logEntry, FILE_APPEND);
...
// Поле source_description -> SOURCE_DESCRIPTION
$v = $_POST['source_description'] ?? '';
if (is_array($v) && isset($v['value'])) $v = $v['value'];
if (is_array($v)) $v = reset($v) ?: '';
if (is_scalar($v) && trim((string)$v) !== '') $fields['SOURCE_DESCRIPTION'] = trim((string)$v);
...
if ($deliveryFailed) {
    ...
    $failedFile = __DIR__ . '/../logs/bryansk_yad_traffix_failed.json';
