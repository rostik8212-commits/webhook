<?php
require 'config.php';
// Ростов. Наш ВК. посты — ВКонтакте (Lead Forms) — 26.02.2026 16:11
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: Ростов. Наш ВК. посты\n";
    $securityLog .= "Hook: rostov_nash_vk_posty\n";
...
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    ...
    file_put_contents(__DIR__ . '/../logs/rostov_nash_vk_posty.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/rostov_nash_vk_posty.log', $logEntry, FILE_APPEND);
...
if ($deliveryFailed) {
    ...
    $failedFile = __DIR__ . '/../logs/rostov_nash_vk_posty_failed.json';
