<?php
require 'config.php';
// Иваново.КЦ.Мир без долгов — КЦ — 26.02.2026 16:08
...
$hookName = 'ivanovo_kts_mir_bez_dolgov';
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: Иваново.КЦ.Мир без долгов\n";
    $securityLog .= "Hook: ivanovo_kts_mir_bez_dolgov\n";
...
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    ...
    file_put_contents(__DIR__ . '/../logs/ivanovo_kts_mir_bez_dolgov.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/ivanovo_kts_mir_bez_dolgov.log', $logEntry, FILE_APPEND);
...
if ($deliveryFailed) {
    ...
    $failedFile = __DIR__ . '/../logs/ivanovo_kts_mir_bez_dolgov_failed.json';
