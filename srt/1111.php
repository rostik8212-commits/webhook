...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: 1111\n";
    $securityLog .= "Hook: 1111\n";
...
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    ...
    file_put_contents(__DIR__ . '/../logs/1111.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/1111.log', $logEntry, FILE_APPEND);
...
foreach ($sendResults as $res) {
    ...
    $logFile = __DIR__ . '/../logs/1111_hook" . $res['hook_index'] . ".log';
...
if (count($allFailed) === count($sendResults) && !empty($sendResults)) {
    ...
    $failedFile = __DIR__ . '/../logs/1111_failed.json';
