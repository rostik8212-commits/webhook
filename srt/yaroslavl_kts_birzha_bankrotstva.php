<?php
require 'config.php';
// Ярославль. КЦ Биржа банкротства — КЦ — 26.02.2026 17:29
...
$hookName = 'yaroslavl_kts_birzha_bankrotstva';
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: Ярославль. КЦ Биржа банкротства\n";
    $securityLog .= "Hook: yaroslavl_kts_birzha_bankrotstva\n";
...
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    ...
    file_put_contents(__DIR__ . '/../logs/yaroslavl_kts_birzha_bankrotstva.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/yaroslavl_kts_birzha_bankrotstva.log', $logEntry, FILE_APPEND);
...
if ($deliveryFailed) {
    ...
    $failedFile = __DIR__ . '/../logs/yaroslavl_kts_birzha_bankrotstva_failed.json';
