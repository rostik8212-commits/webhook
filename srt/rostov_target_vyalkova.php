<?php
require 'config.php';
// Ростов. Таргет.Вялкова — Senler — 26.02.2026 18:30
...
$hookName = 'rostov_target_vyalkova';
...
if ($detectedMethod) {
    ...
    $securityLog .= "Интеграция: Ростов. Таргет.Вялкова\n";
    $securityLog .= "Hook: rostov_target_vyalkova\n";
...
if (isset($_REQUEST['test']) || (isset($_REQUEST['action']) && $_REQUEST['action'] === 'test')) {
    ...
    file_put_contents(__DIR__ . '/../logs/rostov_target_vyalkova.log', date('d.m.Y H:i:s') . " " . $logMsg, FILE_APPEND);
    header('Content-Type: application/json; charset=utf-8');
...
if (!$phoneValid) {
    ...
    file_put_contents(__DIR__ . '/../logs/rostov_target_vyalkova.log', $logEntry, FILE_APPEND);
...
if ($deliveryFailed) {
    ...
    $failedFile = __DIR__ . '/../logs/rostov_target_vyalkova_failed.json';
